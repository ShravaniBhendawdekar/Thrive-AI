...
import { getFeedback } from '@/actions/feedback'; // <-- Import the action
...
const Feedback = ({ params }) => {
  ...
  const GetFeedback = async () => {
    try {
      const result = await getFeedback(params?.interviewId); // <-- Use action with param
  ...
  return (
    <div className='p-10'>
      ...
      <Button className='mt-5' onClick={() => router.replace('/interview')}>Back</Button>
    </div>
  );
};
